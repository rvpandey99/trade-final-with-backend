...
router.post("/register", async (req, res) => {
  ...
  if (userIdExist)
    return res
      .status(400)
      .send("User ID already exist. Please try again with different User Id.");
...
router.post("/login", async (req, res) => {
  ...
  const user =
    (await User.findOne({ userId: value.userId.toLowerCase() })) ||
    (await User.findOne({ email: value.userId.toLowerCase() }));
